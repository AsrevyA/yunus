...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
       channel.send(`${member} Adlı Kişi Sunucuya Katıldı **Davet Eden :** ${invite.inviter.tag} Toplam Davet Sayısı (**${total + bonus}**)`)
        }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(`${member.user.tag} Sunucudan Ayrıldı ** Davet Eden:** ${user.tag} Toplam Davet Sayısı (**${Number(total) + Number(bonus)}**)`)
     }
});
//#endregion
